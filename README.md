...
LINEログイン用の`omniauth-line`gemが2021年以降メンテナンスされておらず、

セキュリティ面のリスクがあると判断したため、OAuth2.0 / OpenID Connectの認可コードフローに沿って自前で実装しました。

#### 1. stateパラメータによるCSRF対策

認可リクエストごとにランダムな`state`を生成してセッションに保存し、コールバック時に照合することで、

第三者による認可コードの横取り（CSRF）を防いでいます。

照合に使用する`state`のセッションはコールバック直後に破棄しています。
```ruby
# ランダム英数字を付与し、stateに値を保存。
state = SecureRandom.hex(24)
session[:line_oauth_state] = state

# コールバック時、保存したstateを取り出して即削除（再利用防止）
expected_state = session.delete(:line_oauth_state)

# 生成時のstateと照合。不一致なら認証失敗とする
...
#### 2. nonceパラメータによるリプレイ攻撃対策

`state`とは別に`nonce`を生成してセッションに保存し、IDトークン検証時にLINEへ渡して照合しています。

`nonce`は傍受されたIDトークンの使い回し（リプレイ攻撃）を防いでします。

```ruby
# ランダム英数字を付与し、nonceに値を保存。
nonce = SecureRandom.hex(24)
session[:line_oauth_nonce] = nonce
```

#### 3. 認可コードフロー（サーバー間でのトークン交換）

コールバックで受け取った認可コードを、サーバー間通信でアクセストークン/IDトークンに交換しています。

`client_secret`をフロントに露出させず、サーバー側だけで扱う認可コードフローを採用することで、トークンの漏洩リスクを抑えています。

#### 4. IDトークンの検証

受けとったIDトークンをそのまま信用せず、LINEのverify APIで検証しています。

このとき`nonce`を渡して照合し、検証が成功した場合のみ`sub`（ユーザー識別子）を取得してログイン処理に進みます。

「トークンを信用する前に必ず検証する」というOpenID Connectの要点を実装しています。

#### 結果
メンテナンスが停止した外部gemへの依存をなくし、OAuth 2.0 / OpenID Connectの
認可コードフローを仕様に沿って自前実装することができました。

---

### 商品登録の負担軽減（フィードバックからの課題解決）

商品を登録するのに11項目入力する必要があり、「多すぎる」というフィードバックを受けました。

このことから、以下の課題が明らかになりました。
- ユーザーが登録時点で離脱する可能性がある
- 入力の手間が大きく、継続的な利用（リピート登録）の妨げになる
- 2step画面に分け心的負担の軽減を試みたが、入力項目自体は変わらず、対症療法にとどまっていた。

これらの課題に対し、以下の対応をしました。

#### 1. 必須項目の削減（11項目 → 5項目）
最低限の情報で商品を識別することができる項目のみを必須とし、店舗やパック単位などの任意項目は

モデルの`optional: true`とマイグレーションの`null: true`に変更しました

あわせて親レコード削除時の挙動を`dependent: :nullify`に変え、「カテゴリを削除しても商品は残る」よう柔軟な構造にしました。

#### 2. 1画面でのシンプルな登録フォーム

必須項目を5項目に絞ったことで、MVPで分けていた2step画面を1画面に統合しました。

入力項目数そのものを減らしたため、画面遷移なしで登録が完結します。

また、より細かく登録したい場合は、必須項目の登録後に完了画面から詳細情報（店舗・カテゴリーなど）の追加へ進める導線を用意しました。

必須入力と任意入力を画面ごとで分離することで、登録直後に迷わず次の行動を選べるようにしています。

| 商品登録完了画面 | 
|:---:|
| <img src="https://i.gyazo.com/527e756401d9bf96ab8aef2d51d37935.png" width="200"> |
| 商品を登録すると、`詳細情報`・`一覧画面に戻る`・`もう1件登録する`の選択肢を提示し、ユーザーが次何をすべきか迷わず選択可能 |

#### 3. 同一商品の2回目以降を自動入力する仕組み
`content_units`（内容量単位）と`pack_units`（包装単位）の表記揺れを防ぎユーザーが管理しやすいようマスタテーブルに正規化しました。

その上で、商品名の入力時点で前回値の登録した`content_quantity`（内容量）と`content_units`（内容量単位）を自動的に反映する仕組みを整えユーザーの入力負担を軽減しました。

| 同一商品2回目登録時 | 
|:---:|
| <img src="https://i.gyazo.com/a9677373f06819baf55e989e65cda375.gif" width="200"> |
| 前回登録した商品を入力することで、`内容量`と`内容量単位`を自動的に反映 |

#### 結果
必須項目を11項目から5項目に削減し最低限の入力で登録が可能になりました。また、同一商品の再登録は商品名の入力だけで内容量・単位が自動入力されるようになり、ユーザーの入力負担を可能な限り削減しました。
